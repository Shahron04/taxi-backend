...
// ═══════════════ EVENT DELEGATION — все кнопки через data-атрибуты ═══════════════
// Вешаем ОДИН обработчик на document, ловим клики по кнопкам с data-action
document.addEventListener('click', function(e){
  ...
  switch(action){
    case 'order':      quickOrder(car); break;
    case 'chat':       openChat(car);   break;
    case 'balance':    doAddBalance(car); break;
    case 'bonus':      doAddBonus(car);   break;
    case 'remove':     doRemove(car);     break;
    case 'delete':     deleteDriver(car); break;
    case 'info':       showModalByCar(car); break;
    case 'approve-pin':    approvePin(pinId); break;
    case 'reject-pin':     rejectPin(pinId);  break;
    case 'approve-bal':    approveBalance(parseInt(id)); break;
    case 'reject-bal':     rejectBalance(parseInt(id));  break;
    case 'chat-driver':    openChat(car); break;
  }
});

function mkBtn(cls, icon, action, car, extra){
  // extra = объект дополнительных data-атрибутов
  let attrs = `class="btn ${cls}" data-action="${action}" data-car="${esc(car)}"`;
  if(extra) Object.entries(extra).forEach(([k,v])=>{ attrs += ` data-${k}="${v}"`; });
...
function renderDriversTable(){
  ...
  tbody.innerHTML = keys.map(k=>{
    ...
    const car = d.car_number||k;
    const r = state.ratings[car] || {avg:5.0, count:0};
    const statusBadge = d.status==='free'
      ? `<span class="badge badge-free">🟢 Свободен</span>`
      : `<span class="badge badge-busy">🔴 На заказе</span>`;
    return `<tr>
      <td><span class="car-num">${esc(car)}</span></td>
      <td>${esc(d.driver_name||'—')}<br><small style="color:var(--muted)">${esc(d.phone||'')}</small></td>
      <td>${statusBadge}</td>
      ...
      <td style="white-space:nowrap">
        ${mkBtn('btn-primary','📦','order',car)}
        ${mkBtn('btn-info','💬','chat',car)}
        ${mkBtn('btn-success','💰','balance',car)}
        ${mkBtn('btn-orange','🎁','bonus',car)}
        ${mkBtn('btn-ghost','ℹ️','info',car)}
        ${mkBtn('btn-danger','🗑','remove',car)}
      </td>
    </tr>`;
  }).join('');
}

function showModalByCar(car){
  const d = state.drivers[car] || {};
  const r = state.ratings[car] || {avg:5.0};
  showModal(car, d.driver_name||'—', d.phone||'—', d.status||'', d.balance||0, d.speed||0,
            r.avg||5, state.bonuses[car]||0, state.revenue[car]||0);
}
...
function updateDriverSelect(){
  ...
  Object.values(state.drivers).forEach(d=>{
    ...
    opts.push(`<option value="${esc(d.car_number)}">${icon} ${esc(d.car_number)} — ${esc(d.driver_name||'—')}</option>`);
  });
...
async function loadDriversPage(){
  ...
  document.getElementById('tbody-pins').innerHTML = pins.map(p=>{
    ...
    const actions = p.status==='pending' ? `
      <button class="btn btn-success" data-action="approve-pin" data-car="${esc(p.car_number)}" data-pin="${esc(p.pin_id)}">✅ Одобрить</button>
      <button class="btn btn-danger"  data-action="reject-pin"  data-car="${esc(p.car_number)}" data-pin="${esc(p.pin_id)}">❌ Отказ</button>
    ` : '—';
    return `<tr>
      <td>${esc(p.name)}</td>
      <td>${esc(p.phone)}</td>
  ...
  try{
    ...
    document.getElementById('tbody-bal-reqs').innerHTML = bReqs.map(r=>{
      ...
      const acts = r.status==='pending' ? `
        <button class="btn btn-success" data-action="approve-bal" data-id="${r.id}">✅</button>
        <button class="btn btn-danger"  data-action="reject-bal"  data-id="${r.id}">❌</button>
      ` : '—';
      return `<tr>
  ...
  document.getElementById('tbody-all-drivers').innerHTML = all.map(d=>{
    ...
    return `<tr>
      ...
      <td><span class="pin-code" style="font-size:14px">${esc(d.pin)}</span></td>
      <td style="color:var(--green);font-weight:600">${fmt(d.balance)} сум</td>
      <td>⭐ ${r.avg} <small style="color:var(--muted)">(${r.count})</small></td>
      <td style="color:var(--orange);font-weight:600">${fmt(rev)} сум</td>
      <td>
        <button class="btn btn-success" data-action="balance" data-car="${esc(d.car_number)}">💰</button>
        <button class="btn btn-orange"  data-action="bonus"   data-car="${esc(d.car_number)}">🎁</button>
        <button class="btn btn-danger"  data-action="delete"  data-car="${esc(d.car_number)}">🗑</button>
      </td>
    </tr>`;
  ...
  container.innerHTML = sorted.map(([car,r],i)=>{
    ...
    return `<div class="driver-card">
      ...
      <div style="display:flex;flex-direction:column;gap:5px">
        <button class="btn btn-orange" data-action="bonus" data-car="${esc(car)}">🎁 Бонус</button>
        <button class="btn btn-info"   data-action="chat"  data-car="${esc(car)}">💬 Чат</button>
      </div>
    </div>`;
  }).join('');
}
...
function loadChatDriverList(){
  ...
  // Строим DOM через createElement — никаких onclick в строках
  container.innerHTML = '';
  keys.forEach(k=>{
    const d = state.drivers[k];
    const car = d.car_number||k;
    const icon = d.status==='free' ? '🟢' : '🔴';
    const isActive = car === currentCar;
    const el = document.createElement('div');
    el.style.cssText = `padding:9px 10px;border-radius:var(--r);cursor:pointer;background:${isActive?'var(--bg4)':'transparent'};transition:background .15s;display:flex;align-items:center;gap:8px`;
    el.dataset.action = 'chat-driver';
    el.dataset.car = car;
    el.innerHTML = `<span>${icon}</span><div>
      <div style="font-size:13px;font-weight:600;color:var(--gold)">${esc(car)}</div>
      <div style="font-size:11px;color:var(--muted)">${esc(d.driver_name||'—')}</div>
    </div>`;
    el.addEventListener('mouseenter', ()=>{ if(car!==currentCar) el.style.background='var(--bg4)'; });
    el.addEventListener('mouseleave', ()=>{ if(car!==currentCar) el.style.background='transparent'; });
